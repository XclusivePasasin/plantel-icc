<template>
    <div class="px-4 py-5 my-5">
        <div class="col-lg-6 mx-auto text-center text-dark shadow-none p-0 p-md-3">
            ...
            <!-- Template: Añadir v-if al select -->
            <select
            name="turno"
            id="turno"
            v-if="!isHiddenPage"
...
<script>
    export default {
       ...
       isHiddenPage() {
            ...
            'Hoja de Control en Proceso',
            'Otra página a ocultar',
            ]);
            const urlParams = new URLSearchParams(window.location.search);
            return hiddenPages.has(urlParams.get('page_pass'));
        }
        },
        methods: {
            sendValue: function(){
                this.$emit('load-order',this.search_val.trim()+","+this.turno);
            },
            enterKey: function(e){
                if(e.key === 'Enter') {
                    this.sendValue();  
            ...
            goActiveOrder(id){
                console.log(id,this.turno)
                this.$emit('load-order',id+","+this.turno);
            },
                    ...
                    case 2:
                        {
                            ...
                            if(this.typeorder == "mix"){
                                message = "Pendiente de aprobación de pesaje";
                            }
                    ...
                    case 3:
                        {
                            let message = "";
                            if(this.typeorder == "mix"){
                                message = "Pesaje aprobado (Pendiente de revisar mezcla)";
                            }
                            if(this.typeorder == "packing"){
